...
# Example 1: 4.28
```{r}
tempo<-read.table("D:/mathbooks/multianalysis/calculate/ch1ex5.dat")
Airp<-as.data.frame(tempo)
#v2 is Solar radiation
Solar.radia<-sort(Airp[,2])
```

```{r}
#Solve normal quantiles
normq<-qnorm(ppoints(length(Solar.radia)))
```

```{r}
#mapping 
#log.Solar.radia<-log(Solar.radia)
qqnorm(Solar.radia,main = "solar radiation")
qqline(Solar.radia)
```

```{r}
cor(Solar.radia,normq)
```
...
```{r echo=FALSE}
#T1.9data
temp<-read.table("D:/mathbooks/multianalysis/data/JohnsonWichern Data sets/JohnsonWichern Data sets/T1-9l.txt")
record_temp<-as.data.frame(temp)
```
...
#100m
```{r}
qqnorm(record$`100m(m/s)`)
qqline(record$`100m(m/s)`)
```

```{r}
norm_test(record$`100m(m/s)`)
```
...
#200m
```{r}
qqnorm(record$`200m(m/s)`)
qqline(record$`200m(m/s)`)
```

```{r}
norm_test(record$`200m(m/s)`)
```
rQ=0.9859>0.9783.

#400m
```{r}
qqnorm(record$`400m(m/s)`)
qqline(record$`400m(m/s)`)
```

```{r}
norm_test(record$`400m(m/s)`)
```
rQ=0.9846>0.9783.

#800m
```{r}
qqnorm(record$`800m(m/s)`)
qqline(record$`800m(m/s)`)
```

```{r}
norm_test(record$`800m(m/s)`)
```
...
#1500m
```{r}
qqnorm(record$`1500m(m/s)`)
qqline(record$`1500m(m/s)`)
```

```{r}
norm_test(record$`1500m(m/s)`)
```
rQ=0.9474<0.9783.

#3000m
```{r}
qqnorm(record$`3000m(m/s)`)
qqline(record$`3000m(m/s)`)
```

```{r}
norm_test(record$`3000m(m/s)`)
```
rQ=0.9301<0.9783.

#Marathon
```{r}
qqnorm(record$`Marathon(m/s)`)
qqline(record$`Marathon(m/s)`)
```

```{r}
norm_test(record$`Marathon(m/s)`)
```
rQ=0.9221<0.9783.
